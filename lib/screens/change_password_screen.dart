...
class _ChangePasswordScreenState extends State<ChangePasswordScreen> {
  ...
  bool get _passwordsMatch => _newCtrl.text == _confirmCtrl.text && _newCtrl.text.isNotEmpty;
  ...
  Future<void> _save() async {
    ...
    if (!_hasMinLength || !_hasNumber || !_hasUpper) {
      _snack('New password does not meet requirements', isError: true); return;
    }
    if (!_passwordsMatch) {
  ...
  Widget _lockIcon() => Container(
    ...
    decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: AppColors.green.withOpacity(.15), blurRadius: 20, offset: const Offset(0, 8))]),
    child: const Icon(Icons.lock_outline_rounded, size: 34, color: AppColors.green));
  ...
  Widget _fieldsCard() => Container(
    ...
    child: Column(children: [
      ...
      _passwordField('New Password', _newCtrl, _showNew,
          () => setState(() => _showNew = !_showNew), onChange: (_) => setState(() {})),
      const Divider(height: 1, color: AppColors.border),
      _passwordField('Confirm New Password', _confirmCtrl, _showConfirm,
          () => setState(() => _showConfirm = !_showConfirm), onChange: (_) => setState(() {})),
    ]));
    ...
    child: Row(children: [
      AnimatedContainer(duration: const Duration(milliseconds: 200),
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: met ? AppColors.green : AppColors.border, width: 1.5)),
        child: met ? const Icon(Icons.check_rounded, size: 12, color: Colors.white) : null),
